...
// Date: Fri Sep 22 11:18:35 2023
// Arguments:
// "-B""macro_default""-W""cpplib:testMulti,all,version=1.0""-T""link:lib""-d""E
// :\MPNP\MyMainFunction\testMulti\for_testing""-v""E:\MPNP\MyMainFunction\testM
// ulti.m"
//

#define EXPORTING_testMulti 1
#include "testMulti.h"
    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("testMulti"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return testMultiInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "testMulti", nlhs, plhs, nrhs, prhs);
}

LIB_testMulti_CPP_API 
void MW_CALL_CONV testMulti(int nargout, mwArray& cc, mwArray& dd, const mwArray& aa, 
                            const mwArray& bb)
{
    mclcppMlfFeval(_mcr_inst, "testMulti", nargout, 2, 2, &cc, &dd, &aa, &bb);
}
