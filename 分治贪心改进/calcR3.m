%% 使用分治贪心关联获取线序号 然后获取时延关系解算，这是分治贪心改进的算法啊
% 加上去除异常平台
% 传入参数：角度，平台数目，平台位置，目标数 当前时刻  观测周期
% arrR 5000*3 cell 时间 目标 平台数
function [outTimeM, choose] = calcR3(sourceAll, timeR, arrR, pNum, node, num, t_obs, T)
tic;
var2d = 1.5^2;
var2 = var2d * (pi / 180)^2;
c = 1500;
...
%% ==========================分治贪心关联===============================
dPonit = 10;
PD = 0.9; %0.9        	% 检测概率2~3个目标时是0.99，4~5个目标时是0.9
Fai = 2 * pi;
M = 3; % 选取最少线点数
Q = 10; % 列表最大长度
I = 3; % 并行次优条数
...
for iii = 1:length(t_obs)
    ...
    if num == 2 && pNum == 2
        ...
        if all(ns == 1)
            ...
            choose{iii} = outLocs;
            %             [choose{iii}, sortedIdx] = sortCellArrayByFirstElement(choose{iii});
            outLocX = outDCGT(:, length(Z)+1);
            outLocY = outDCGT(:, length(Z)+2);
            %             outLocX = outLocX(sortedIdx);
            %             outLocY = outLocY(sortedIdx);
        end
    end
...
% outLoctionSPCX = outLoctionCAX;
% outLoctionSPCY = outLoctionCAY;
...
for iii = 1:length(t_obs)
    ...
    for i = 1:num
        ...
        % 计算r* theta
        % 计算两点间的距离
        distances = sqrt(sum((mean(res{3}) - node).^2, 2));

        % 获取最大距离
        max_dis = max(distances);
        %         max_dis = 1000;
        L = 2 * 0.2 / 180 * pi * max_dis; % 一般是六七十
        mea = 1e9 * ones(3, 2);
        mea(3, :) = mean(res{3});
        mea(2, :) = mean(res{2});
        mea(1, :) = mean(res{1});
        ...
        if any(yita < 1)
            ...
            locpie_new = locpie; % 用于删减关联错的平台的结果
            ...
            if ~isempty(nonSubsetElementsInRes)
                ...
                % 删除对应平台关联
                for numm = 1:size(nonSubsetElementsInRes)
                    str = nonSubsetElementsInRes(numm); % 示例字符串
                    numbers = regexp(str, '\d+', 'match'); % 提取出所有的数字部分
                    lastNumber = numbers{end}; % 将最后一个数字转换为数值类型
                    disp(lastNumber); % 显示最后一个数字

                    for ij = 1:size(locpie_new, 1)
                        row = locpie_new{ij, 1}; % 获取当前行的数据
                        if any(endsWith(row, lastNumber)) % 检查当前行是否有元素以 "1" 结尾
                            results = [results, ij]; % 如果有，则将行索引添加到结果中
                        end
                    end
            ...
            totalRows = size(respie, 1); % 获取总行数
            otherIndices = setdiff(1:totalRows, results); % 可以选用的结果
%             if size(otherIndices, 2) == 0
%                 respie_new = respie;
%                 if size(respie_new, 1) > 1
%                     new_res = mean(respie_new);
%                 else
%                     new_res = respie_new;
%                 end
%             else
%                 respie_new = respie(otherIndices);
%                 if size(respie_new, 1) > 1
%                     new_res = mean(respie_new);
%                 else
%                     new_res = respie_new;
...
%             end
            ...
            new_mean_res{iii, i} = new_res;
            new_mean_resX(iii, i) = new_res(1);
            new_mean_resY(iii, i) = new_res(2);


        else
            % 无操作
            mean_res{iii, i} = mean(mea);
            mean_resX(iii, i) = mean(mea(:, 1));
            mean_resY(iii, i) = mean(mea(:, 2));

            new_mean_res{iii, i} = mean(mea);
            new_mean_resX(iii, i) = mean(mea(:, 1));
            new_mean_resY(iii, i) = mean(mea(:, 2));
        end
    end
end
...
%     = myNNDAQZ(cellToNanFilledMatrix4x1(mean_res(iii,:)),targetnum,L,Lth,disth,0.1,Z_NNDAforward, Z_kalmanforward, ...
...
% endZkforward=endZk;
% % disp(Z_NNDA)
% filter_res(:, end + 1) = Z_NNDA(:,end);
% end
% % 把Z_NNDA搞到mean_res中去
% if filter_res(1, 1) < 5500
%
% else
%     % 定义新的行顺序
%     newOrder = [3 4 1 2];
%
%     % 重新排列行
%     filter_res = filter_res(newOrder, :);
% end
% filter_res = matrixToCellArrayColsV2(filter_res);

%% 计算定位误差
% 1. 提取位置
% 计算误差
% for i = 1:num
% %     if i == 1 % 5000，4000与6000，3000参数
% %         k = 27; % 这个值依据主函数angM的第一个元素与angR的最先到达的平台第一个非零元素的相隔的周期号决定
% %     else
% %         k = 37;
% %     end
% % 仿真2的参数 4000,4000Yu 7000,4000参数
%     if i == 1
%         k = 32; % 这个要根据到达当前平台的时刻确定
%     else
%         k = 37;
%     end
...
%         if ~isempty(B)
%             TDOA_distances{i}(iii, 1) = sqrt(sum((~isempty(A) & ~isempty(B)).*(A-B).^2, 2));
%         else
%             TDOA_distances{i}(iii, 1) = nan;
%         end
%         B = [outLoctionSPCX(i, iii)', outLoctionSPCY(i, iii)'];
%         if ~isempty(B)
%             AOA_distances{i}(iii, 1) = sqrt(sum((~isempty(A) & ~isempty(B)).*(A-B).^2, 2));
%         else
%             AOA_distances{i}(iii, 1) = nan;
%         end
%
%         B = cell2mat(TA_res(iii, i));
%         if ~isempty(B)
%             TA_distances{i}(iii, 1) = sqrt(sum((~isempty(A) & ~isempty(B)).*(A-B).^2, 2));
%         else
...
figure('Units', 'centimeters', 'Position', [10, 10, 14, 10 / 4 * 3]); % 左下宽高

hold on
for ii = 1:num
    plot(outLoctionSPCX(ii, :), outLoctionSPCY(ii, :), '.', 'DisplayName', ['目标', num2str(ii)])
end
axis([0, 10e3, 0, 10e3])
...
legend('show', 'Location', 'eastoutside'); % 显示图例
...
hold on
for ii = 1:num
    plot(TA_resX(:, ii), TA_resY(:, ii), '.', 'DisplayName', ['目标', num2str(ii)])
end
axis([0, 10e3, 0, 10e3])
...
hold on
for ii = 1:num
    plot(TDOA_resX(:, ii), TDOA_resY(:, ii), '.', 'DisplayName', ['目标', num2str(ii)])
end
axis([0, 10e3, 0, 10e3])
...
hold on
for ii = 1:num
    plot(mean_resX(:, ii), mean_resY(:, ii), '.', 'DisplayName', ['目标', num2str(ii)])
end
axis([0, 10e3, 0, 10e3])
...
hold on
for ii = 1:num
    plot(new_mean_resX(:, ii), new_mean_resY(:, ii), '.', 'DisplayName', ['目标', num2str(ii)])
end
axis([0, 10e3, 0, 10e3])
title("分治贪心关联抗异常参量定位_新", 'FontSize', 10)
scatter(node(:, 1), node(:, 2), 'b^', 'filled', 'LineWidth', 0.5, 'SizeData', 100, 'DisplayName', '观测站');
% legend('目标1', '目标2', '观测站', 'FontSize', 10)
% legend('目标1', '目标2', '目标3', '观测站', 'Location', 'eastoutside', 'FontSize', 12)
hold off
...
% %         legend('AOA', 'TDOA/AOA', '抗异常参量后结果','FontSize', 10)
%
...
function kf = initKalmanFilter(x0, P0, R, Q, dt)
% 初始化卡尔曼滤波器结构体
kf.x = x0; % 初始状态估计
kf.P = P0; % 初始协方差矩阵
kf.R = R; % 观测噪声协方差
kf.Q = Q; % 过程噪声协方差
kf.F = [1, 0, dt, 0; 0, 1, 0, dt; 0, 0, 1, 0; 0, 0, 0, 1]; % 状态转移矩阵
kf.H = [1, 0, 0, 0; 0, 1, 0, 0]; % 观测矩阵
end

function kf = kalmanPredict(kf)
% 预测步骤
kf.x = kf.F * kf.x; % 预测状态
kf.P = kf.F * kf.P * kf.F' + kf.Q; % 预测协方差
end

function [kf, y] = kalmanUpdate(kf, z)
% 更新步骤
y = z - kf.H * kf.x; % 测量残差
S = kf.H * kf.P * kf.H' + kf.R; % 残差协方差
K = kf.P * kf.H' / S; % 卡尔曼增益
kf.x = kf.x + K * y; % 更新状态估计
kf.P = (eye(size(kf.P)) - K * kf.H) * kf.P; % 更新协方差
end
...
function cellArray = matrixToCellArrayCols(M)
% 检查输入矩阵是否每列都是4元素
if size(M, 1) ~= 4
    error('Each column of the input matrix must have exactly 4 elements.');
end
...
% 初始化cell数组，每列两个cell
cellArray = cell(2, numCols);

% 循环处理每一列
for i = 1:numCols
    cellArray{1, i} = M(1:2, i)'; % 处理前两个元素
    cellArray{2, i} = M(3:4, i)'; % 处理后两个元素
end
end
function cellArray = matrixToCellArrayColsV2(M)
% 检查输入矩阵是否每列都是4元素
if size(M, 1) ~= 4
    error('Each column of the input matrix must have exactly 4 elements.');
end
...
% 初始化cell数组，大小为N x 1，每个cell内再包含一个2 x 1的cell
cellArray = cell(numCols, 2);

% 循环处理每一列
for i = 1:numCols
    %         innerCell = cell(1, 2);  % 每个cell内部是2x1的cell
    %         innerCell{1} = M(1:2, i)';  % 处理前两个元素
    %         innerCell{2} = M(3:4, i)';  % 处理后两个元素
    cellArray{i, 1} = M(1:2, i)'; % 将内部cell赋值给外部cell的对应位置
    cellArray{i, 2} = M(3:4, i)'; % 将内部cell赋值给外部cell的对应位置
end
end
function [V_sorted, sortedIdx] = sortCellArrayByFirstElement(V)
% sortCellArrayByFirstElement 对包含向量的 cell 数组进行排序
% 这个函数将根据每个向量的第一个元素对 cell 数组进行排序
% 同时返回排序索引，以便用于其他数据的同步排序


% 提取每个向量的第一个元素
firstElements = arrayfun(@(x) x(1, 1), V(:, 1));

% 获取排序索引
[~, sortedIdx] = sort(firstElements);

% 使用排序索引来重新排列 cell 数组
V_sorted = V(sortedIdx, :);
end
