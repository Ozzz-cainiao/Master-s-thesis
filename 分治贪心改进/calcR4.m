%% 使用分治贪心关联获取线序号 然后获取时延关系解算，这是分治贪心改进的算法啊 加上异常判据，有主函数3调用，作为仿真1
% 传入参数：角度，平台数目，平台位置，目标数 当前时刻  观测周期
% arrR 5000*3 cell 时间 目标 平台数
function [outTimeM, choose] = calcR4(sourceAll, timeR, arrR, pNum, node, num, t_obs, T)
    ...
    for i = 1:num
        ...
        if any(yita < 1)
            ...
            if ~isempty(nonSubsetElementsInRes)
                ...
                % 剔除对应平台的参量
                disp(['关联出错的平台为', nonSubsetElementsInRes(end)]);
                str2double(nonSubsetElementsInRes(end));
            end
        else
            % 无操作
...
legend('目标1', '目标2', '观测站', 'FontSize', 10, 'Location', 'northwest')
...
scatter(node(:, 1), node(:, 2), 'b^', 'filled', 'LineWidth', 0.5, 'SizeData', 100);
legend('目标1', '目标2', '观测站', 'FontSize', 12, 'Location', 'northwest')
% legend('目标1', '目标2', '目标3', '观测站', 'Location', 'eastoutside', 'FontSize', 12)
hold off
set(gca, 'Box', 'on')
...
%% 画出误差分布图
for i = 1:num
    figure('Units', 'centimeters', 'Position', [10, 10, 16, 9]); % 左下宽高
    %     figure
    hold on;
    plot(AOA_distances{i}, '-+');
    plot(TDOA_distances{i}, '-o');
    ...
    legend('AOA', 'TDOA', 'TDOA/AOA', '抗异常参量后结果', 'FontSize', 10, 'Location', 'eastoutside')
