...
%                 % 虚警期望
%                 lamda = 0;
%                 num_false_alarms = poissrnd(lamda);
%                 % 生成均匀分布的虚警信号值
%                 while num_false_alarms > 0
%                     angR{j}(i, end + 1) = randi([-180, 180]);
%                     timeR{j}(i, end + 1) = rand + randi([2, 7]);
...
legend('目标1', '目标2', '观测站', 'FontSize', 10) %'Location', 'eastoutside' ,
...
[outTimeM, choose] = calcR3(sourceAll, timeM, angM, numOfPlatForm, node, numOfSource, t_obs, T);
