function varargout = mdada(varargin)
%mdada 基于角度的SD关联
% ZLM自用版本
...
if max(ns) > 3
    TH1  = outK1(1,S+3);
    % 对矩阵outK1中每一行的前四列进行查找操作，找到每一行中非零元素的位置索引，
    % 并将这些索引存储在一个cell数组temp中
    temp = arrayfun(@(x) find(outK1(x,1:S)),1:size(outK1,1),'un',0);   
    % temp中找到长度为2的元素所对应的outK1矩阵中的值，并将这些值中的最小值赋给TH1变量
    outK2 = outK1(cellfun(@length,temp) > 1, :); % 提取出至少有2个平台参与解算的解
%     TH1  = min(outK1(cellfun(@length,temp) == 1, S+3)); % 找到至少有2个平台参与测量的解
    outK3 = outK1(find(~isnan(outK1(:, S + 3)) & ~isinf(outK1(:, S + 3))), :);
    TH1  = outK3(Q / 2, S+3); % 找到至少有2个平台参与测量的解
end
