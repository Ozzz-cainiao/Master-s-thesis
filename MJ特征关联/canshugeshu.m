...
plot_zhengquelv=zeros(length(PNR),length(canshu));
for len=1:length(PNR)
NA=sqrt(X.^2/(10^(PNR(len)/10)));



 shiyancishu=1000;%试验次数，蒙特卡洛

%  fenleishu_right=0;%分类正确次数
 for len2=1:length(canshu)
     testttt=0;
      hh=0;
      Xrec=zeros(R,canshu(len2));
      fenleishu_right=0;%分类正确次数
...
for i=1:R
Rabrec=cacldetla_plie(Xi,R,canshu(len2),i);
Rab(i,:)=Rabrec(i,:);
end
%  end
...
hh=hh+1;
 ...
 plot_zhengquelv(len,len2)=fenleishu_right/shiyancishu;
 end
end
figure 

% plot(canshu,plot_zhengquelv)
% hold on
plotx=[0,canshu];
ploty=[0,plot_zhengquelv];
plot(plotx,ploty);
% plot(0,0)
axis([0,length(canshu),0,1.6])
% title('30dB下目标分类正确率与参数个数关系');
xlabel('目标特征参数个数/个')
ylabel('目标关联正确率')
