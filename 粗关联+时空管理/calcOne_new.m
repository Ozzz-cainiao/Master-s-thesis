...
%% 传入参数：角度，平台数目，平台位置，目标�? 当前时刻  观测周期
% ����û�мӸ���
function [outLoctionCAX, outLoctionCAY] = calcOne(arrR, pNum, node,num,t_obs)
array = cell2mat(arrR);
% disp(array);
% disp(pNum);
% disp(node);
...
%     disp(['正在处理', num2str(iii)])
    ...
    if num == 2 && pNum == 2
        if all(ns ~= 1) % 无解只剩下两个平台有测量，不能进行关联，只能对一个目标进行处�?
...
%     end
%     di(isnan(di)) = 1e8;
%     if size(di, 1) > size(di, 2) % HungarianAlgorithm.m只能对列数≥行数的正确关�?
...
%                 r_e = sqrt(x_e.^2+y_e.^2); % 估计位置与观测站之间的距�?
%                 t_e = r_e / c;
%                 t_ed = round(t_e-t_e(1), 1); % 时延�?
...
PD = varargin{3}(2); % �?测概�?
Fai = varargin{3}(3);
M       = varargin{4}(1); % 选取�?少线点数
Q = varargin{4}(2); % 列表�?大长�?
I = varargin{4}(3); % 并行次优条数
pNum = size(node, 1); % 节点�?
angM = arrayfun(@(x)cat(2, cell2mat(x{1, 1}(:, 1))), Z, 'un', 0); % 角度特征元胞�?
%%% ==========================基于分治思想的最优交点集合�?�取===============================
%%% Step1：生成交�?
...
iter = num2cell(cat(2, temp{:})); % �?有组�?
Niter = size(iter, 1);
temp = arrayfun(@(x) find(cell2mat(iter(x, :))), 1:Niter, 'un', 0); % 中间变量
iter1 = iter(cellfun(@length, temp) == 1, :); % �?有线的序数组
iter2 = iter(cellfun(@length, temp) == 2, :); % �?有两两相交的组合
K = cell(size(iter2, 1), 2); % 预分�?
for k = 1:size(iter2, 1) % �?次循环替代嵌�?
    locs1 = find(cell2mat(iter2(k, :))); % �?选平�?
...
iter22 = cat(2, iter2, K); % �?选平台测量序�?+交点矩阵
%%% Step2：对每条线�?�取�?短的M条线�?
W = {};
for k = 1:size(iter1, 1)
    locs21 = find(cell2mat(iter1(k, :))); % �?选平台序�?
    locsM = iter1{k, locs21}; % �?选测量序�?
    Num22 = find(cell2mat(iter22(:, locs21)) == iter1{k, locs21}); % 在iter22筛�?�出来的序号
    point = iter22(Num22, :); % 筛�?�出来的测量序号+位置
    kk = 1;
    kknum = 1;
    locs22 = cell(sum(1:size(point, 1)-1), 3); % 预分�?
    while kk < size(point, 1)
        ...
        for ii = kk + 1:size(point, 1)
            ...
            locs22{kknum, 1} = Num22(kk); % iter22中行序号--对应�?
            locs22{kknum, 2} = Num22(ii); % iter22中行序号--对应�?
            locs22{kknum, 3} = norm(point2-point1); % 距离
            kknum = kknum + 1;
        end
        kk = kk + 1;
    end
    [~, locs23] = sort(cell2mat(locs22(:, 3))); % 按升序排列后的序�?
    if length(locs23) > M
        W = cat(1, W, locs22(locs23(1:M), :)); % W中三列分为iter22中行序号；iter22中行序号；距�?
    else
...
Lamdad = cell(Num3, 2);
for k = 1:Num3
    locs31 = iter22(cell2mat(W(k, 1:2)), :); % locs31�?选测量序号，和交点位�?
    [locs32, locs33] = find(cell2mat(locs31(:, 1:pNum))); % locs32行序�?-》无意义 locs33列序�?-》所选平�?
    if length(unique(locs33)) < 3
        ...
        theta = cell2mat(theta);
        Lamdad{k, 1} = cat(2, locs33, cell2mat(locs35), theta); % 平台序号;测量序号;测量�?
        Lamdad{k, 2} = nan; % 似然
    else
        [locs33, locs34] = unique(locs33); % locs34独一无二数的平台数的序数�? locs33列序数更�?
        locs32 = locs32(locs34); % locs32行序数更�?
        locs35 = arrayfun(@(x, y) locs31(x, y), locs32, locs33); % locs35测量序数，与locs33配合读取测量
        theta = cellfun(@(x, y) angM{x}(y), num2cell(locs33), locs35, 'un', 0);
        theta = cell2mat(theta);
        x3 = node(locs33, 1);
        ...
        thetaE = atan2d(Xd(2)-y3, Xd(1)-x3); % 估计目标位置到平台角�?
        %%% 换成对数似然
        c = zeros(1, pNum);
        for ii = 1:pNum
...
%%% ==========================基于贪心思想的量测集合合�?===============================
Lamdad_W = Lamdad(~isnan(cell2mat(Lamdad(:, 2))), :); % 排除�?个组合有多个同平台测�?
...
while s <= pNum
    ...
    for ii = 1:NumZ
        for jj = 1:Numz
            ...
            locs52 = cellfun(@length, locs51); % 有多�?1个测量的序号
            Mea2 = cellfun(@(x) Mea(x, :), locs51(locs52 > 1), 'un', 0); % 有多�?1个测量的Mea
            locs53 = arrayfun(@(x) unique(Mea2{1, x}(:, 2)), 1:length(Mea2), 'un', 0); % 有多�?1个测量的平台�? 重复测量数量
            locs54 = cellfun(@length, locs53);
            if isempty(find(locs54 > 1, 1))
                [locs55, locs56] = unique(Mea(:, 1));
                if length(locs55) == pNum % 遍历�?有组�?->输出
                    if isempty(W_Plus) % 第一个�??
    ...
    numZ = 1:NumZ;
    if sum(ismember(numZ, locs61)) ~= NumZ % 未关联的�?
        locs63 = numZ(~ismember(numZ, locs61)); % 未更新的行号
        Z1 = cat(1, Z1, Z{locs63, 1});
    end
    numz = 1:Numz;
    if sum(ismember(numz, locs62)) ~= Numz % 未关联的�?
...
for k = 1:length(W_Plus)
    ...
    B = y6 - x6 .* tand(theta);
    Xd = (A' * A) \ A' * B; % 估计目标位置Xd=[x;y]
    thetaE = atan2d(Xd(2)-y6, Xd(1)-x6); % 估计目标位置到平台角�?
...
for ii = 1:size(W_Plus7, 1)
    ...
    for jj = 1:size(W_Plus7, 1)
        ...
        locs81 = cat(1, W81, W82); % 对两个集合拼�?
        locs82 = arrayfun(@(x) locs81(locs81(:, 1) == x, 2), unique(locs81(:, 1)), 'un', 0); % 平台测量�?
        locs83 = cellfun(@(x) length(unique(x)) == length(x), locs82, 'un', 0); % 平台测量互斥性，如果1不互斥；如果0则互�?
        if length(find(cell2mat(locs83))) == length(locs82) % 如果不互斥的平台�?==有测量平台数
            R8(ii, jj) = 1; % 不互斥的标记�?1
        end
    end
end
...
while q <= size(W_Plus7, 1)
    ...
    P = sum(cat(2, repmat(P, 2, 1), cat(1, zeros(NumPsi, 1), W_Plus7{q, 2}*ones(NumPsi, 1))), 2); % 总似�?
    locsdet = [];
    for ii = NumPsi + 1:NumPsi * 2
        Lq = L(ii, :);
        locs91 = find(Lq == 1)';
        if sum(Lq == 1) == 2 % 存在两个关联量则�?查是否能同时输出
            if R8(locs91(1), locs91(2)) ~= 1 % 不可以同时输�?
                locsdet = cat(1, locsdet, ii);
            end
        elseif sum(Lq == 1) > 2
            locs92 = [locs91(1:end-1), locs91(end) * ones(sum(Lq == 1)-1, 1)]; % 待处理的兼容组合
            if sum(arrayfun(@(x, y) R8(x, y) == 1, locs92(:, 1), locs92(:, 2))) ~= sum(Lq == 1) - 1 % 没有和所有兼�?
                locsdet = cat(1, locsdet, ii);
            end
        end
    end
...
% end
%% 
% function result=calculateOutAngM(angm1,estlocs,pnum) % angm1:1x3cell, estlocs 1x3 double, pnum: 3
%     result=cell(1,pnum);
%     for i=1:pnum
%         result{i}=angm1{i}(estlocs(1,i));
%     end
% end
