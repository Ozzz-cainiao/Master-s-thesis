...
for fram=1:time
    ...
    for index=1:5
        ...
        % [两位帧头 两位源地址 两位目的地址 一位指令类型 一位指令类型的反码 两位数据长度]
        %                     command=typecast(char(101),'uint8');
        %                     recommand=typecast(char((~101)),'uint8');
        %         sdj=typecast(136,'uint16');
        %         newsdj=typecast(sdj,'uint8');
        
        MyByteArray1=[0xA5,0X5A,101,0X00,100,0X00,0X65,0X9A,136,0X00];
        ...
        %先转成单精度
%         temp=single(temp);
        dfjaskj=char(tempvalue.buoyIndex);
        MyByteArray2=typecast(temp,'uint8');%数据部分
        % 帧尾  校验和  需要自己计算出来加上
        
        
        MyByteArray=[MyByteArray1,MyByteArray2];
        checksum=byteCheckSum(MyByteArray,length(MyByteArray));
        MyByteArray=[MyByteArray1,MyByteArray2,checksum];
        fwrite(u1,MyByteArray,'uint8');
...
%% 编写计算帧尾校验和的函数
function test=byteCheckSum(data,length)

    test=uint16(0);
    for i=1:length
        test=test+uint16(data(i));
    end
