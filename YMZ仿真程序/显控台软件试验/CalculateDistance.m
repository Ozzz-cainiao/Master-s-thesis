...
%dr:     目标到原点的距离
%th:     方位以正东为0的弧度
function  [ax, ay, dr, th] = CalculateDistance(long1, lat1, long2, lat2,unit)
    [m_Long_sec1, m_Lat_sec1]=CGeopos(lat1,long1,unit);
    [m_Long_sec2, m_Lat_sec2]=CGeopos(lat2,long2,unit);
    [x1, y1]=GeoToGauss(m_Long_sec1, m_Lat_sec1);  % 原点,黑匣子位置
    [x2, y2]=GeoToGauss(m_Long_sec2, m_Lat_sec2);  % 
    
    dr=sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1));%返回值为米
    dx=x1-x2;%x坐标
    dy=y1-y2;%y坐标
    ax = -dx;
    ay = -dy;
...
%         th=90-th;
%     end
bearingMs = atan2( dx , dy );

if bearingMs < 0
    bearingMs = bearingMs + 2*pi*ceil(-bearingMs/(2*pi));
elseif bearingMs >= 2*pi
    bearingMs = bearingMs - 2*pi*floor(bearingMs/(2*pi));
end
th  = bearingMs/pi*180;
