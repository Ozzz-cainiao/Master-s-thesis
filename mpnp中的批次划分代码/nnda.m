...
% 仿真： 1、改变虚假量测数量nc：公式求取、手动设置 
%       2、改变量测噪声R=[r 0; 0 r]，即r 3、改变虚假量测位置q，偏离真实位置的程度
% 问题： 每次产生杂波时，Sk如何确定？
...
% 最邻近数据关联函数 
% 输入： 
%   Z_Matrix：波门内的有效量测值（包括杂波和真实量测） 
%   PZ_Matrix：有效量测值的误差方差阵
%   Z_Predict：量测预测值 
%   PZ_Predict：量测预测值的误差方差阵 
% 输出： 
%   Z：按照统计距离最近原则关联上的量测值
%   P：关联上的量测值对应的协方差
nm = size(Z_Matrix);
...
% Pzz = H*P_Forward*H'+ R;                   
% S(k+1/k+1) 
% 新息协方差 Pxz = P_Forward*H' ;                       
% 状态与量测之间的协方差 K = P_Forward*H'*(inv(Pzz));               
% K(k+1) 增益
