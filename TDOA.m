...
function [res, loc] = TDOA(timeDelay, node, solveType)
c = 1500;
zs = 0;
...
if solveType == 2 || solveType == 3
    ...
    if v >= 0
        ...
        if solveType == 3
            % 使用第三个平台判解
            if abs(pdist([node(3, :), [EstX(1), EstY(1), 0]])-r(3)) ...
                    > abs(pdist([node(3, :), [EstX(2), EstY(2), 0]])-r(3))
                EstX = EstX(2);
                EstY = EstY(2);
            else
                EstX = EstX(1);
